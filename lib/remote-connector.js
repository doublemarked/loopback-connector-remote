...
var List = require('loopback-datasource-juggler/lib/list');
...
function createProxyMethod(Model, remotes, remoteMethod) {
  ...
  if (original.__isProxy) {
    // Prevent re-proxying of methods
    return;
  }
...
function coerceRemoteDataType(Model, data) {
  if (!data) {
    return data;
  }
  ...
  Object.keys(Model.relations).forEach(function (relationName) {
    ...
    if (data.hasOwnProperty(relationName)) {
      var relatedValue = data[relationName];
